...
4. 想使用 DeepSeek：启动时粘贴 Key；输入过程不会显示字符。分析完成后，“智能解读”会自动使用模型，“生成动态多空解读”需要手动点击，因此不会在普通分析时额外消耗这部分 Token。
5. 不想调用模型：启动提示直接回车。K 线、指标、四 Agent、综合评分、价格区间、仓位和风控仍能正常工作，只是自然语言说明使用本地固定格式。
