...
## 2. 当前小功能：任务 1.2 的项目记忆最小闭环

认知闭环、受控工具和有界工作记忆已经完成。下一小步增加跨一次 Loop 运行的项目记忆，用于保存同一项目内经过明确写入的事实、决定和产物索引，同时阻止不同项目互相读取。最终成果、正式任务状态和验收条件以 `ROADMAP.md` 与 `checklist.json` 为准。

### 必须完成

1. 定义项目记忆条目、项目命名空间和版本化快照的稳定数据契约。
2. 支持显式写入、同键更新、按项目查询和受控删除；禁止把整段运行状态自动永久保存。
3. 严格隔离不同 `project_id`，查询结果不能跨项目泄漏。
4. 支持离线 JSON 持久化和跨运行恢复，并拒绝损坏或不兼容的快照。
5. 允许调用方选择相关项目记忆，以只读形式注入认知 Loop；工作记忆的容量和生命周期保持不变。
6. 为记忆保留类别、来源和创建/更新时间，时间生成方式可在离线测试中固定。
7. 覆盖新增、更新、查询、删除、项目隔离、恢复、损坏快照和 Loop 注入测试。

### 明确不做

- 不在本步实现组织记忆、向量数据库、语义检索或自动知识抽取。
- 不在本步接入 Heartbeat/Cron、Hook、递归目标调度、worktree 隔离或真实 LLM。
- 不补做 Graph 能力，也不扩展金融指标、真实行情或交易能力。
- 不把 API 密钥、账户信息或未经筛选的完整对话永久保存。
