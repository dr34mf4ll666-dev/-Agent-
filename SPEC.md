...
P9 新增版本化证券主数据模块，统一记录证券身份、客户行业、数据源实际需要的分析行业标签、可用模式、能力、快照状态和验证状态。客户前台从主数据生成目录，支持名称/代码搜索和行业筛选；只有 `verified=true` 且 `customer_visible=true` 的标的可以被客户分析和自选功能使用。五粮液（`sz000858`）已通过真实行情、财务、行业、宏观、四 Agent 和完整金融 Graph 验收，作为第一个非银行行业标的进入正式目录。P9 的代码、页面、自动化测试、真实链路和 Chromium 验收均已完成。完整说明见 `docs/security-master.md`。
