...
## 2. 已完成小功能：P9 证券主数据与跨行业扩展

P9 新增版本化证券主数据模块，统一记录证券身份、客户行业、数据源实际需要的分析行业标签、可用模式、能力、快照状态和验证状态。客户前台从主数据生成目录，支持名称/代码搜索和行业筛选；只有 `verified=true` 且 `customer_visible=true` 的标的可以被客户分析和自选功能使用。五粮液（`sz000858`）和华银电力（`sh600744`）已通过真实行情、财务、行业、宏观、四 Agent 和完整金融 Graph 验收，进入正式目录。本轮对华银电力的第二个非银行行业验证属于 P9 之外的独立扩展，不改变 P9 正式验收口径。P9 的代码、页面、自动化测试、真实链路和 Chromium 验收均已完成。完整说明见 `docs/security-master.md`。

明确不做：P9 不把未验证标的展示给客户，不把离线测试行业当作真实行业归属，不让 LLM 参与字段映射或确定性金融计算，不接入真实交易。

### 已完成小功能：P10 数据可信度、可复现与可靠性证据

P10 为每次新分析增加 `DataQualityRuntime.evaluate(snapshot, security_record, now)` 质量评估和运行指纹。质量层逐类保留来源、获取时间、数据对应时间、备用/缓存状态、缺失原因和用户建议，整体输出 `complete`、`degraded` 或 `blocked`。指纹由 `snapshot_id`、证券主数据版本、代码版本、配置版本、模型策略版本和报告版本规范化后计算 SHA-256；它不保存 Prompt、API Key 或完整原始行情。

SQLite 已从 v3 迁移到 v4，新增质量/身份摘要表；旧报告仍可读取，缺少新字段时显示“历史报告，来源版本未知”，不把旧记录伪装成可直接比较。报告比较新增 `change_reasons`，区分行情时间、数据源状态、证券主数据、确定性规则和大模型解释策略变化。

客户普通版增加“本次分析可信度”卡片，专业版增加数据质量明细、版本输入和运行指纹；分析结束后加载区域只保留结果或错误。管理员后台登记“面试展示 · 可信度与可靠性”入口，命令行入口 `Scripts/demo_interview_showcase.py` 固定展示正常运行、数据源超时重试、缓存降级、Checkpoint 恢复和输出校验失败，并输出成功率、恢复率、重试/降级、缓存、重复节点、模型调用、Token、P50/P95/P99 和差异原因。

明确不做：P10 不把固定离线实验写成线上 SLA，不通过并发优化掩盖数据契约问题，不改变确定性指标/评分/仓位/风控，不接入真实交易，不把 Harness、Checkpoint 和原始日志暴露给客户普通版。

### 已完成基线：P5 统一可观测性和可靠性面板
...
当前 A–D 正式任务与 P1–P10 均已完成当前计划范围。P9 使用 `SecurityMasterRuntime` 统一证券目录、行业和能力状态，并通过五粮液和华银电力真实完整 Graph 验收两个非银行行业；P10 使用 `DataQualityRuntime`、运行指纹、SQLite v4、可靠性离线演示和双层可信度投影补齐面试展示证据。P8 使用 `DeploymentRuntime` 统一版本、健康、就绪和启动阻断，使用 `SecurityRuntime` 统一登录、客户/管理员权限、CSRF、分级限流、安全审计和按会话保存的 DeepSeek Key；Dashboard 暴露状态、认证与安全管理接口。Docker 镜像以非 root 用户运行，Compose 只映射本机端口、使用只读根文件系统和持久卷。CI 覆盖 Linux/Windows、Ruff、mypy、覆盖率、密钥与依赖扫描、真实 Chromium、镜像构建和容器重启恢复。真实交易始终关闭。后续增加更多行业或资产时，必须继续通过主数据验证和真实链路验收。
