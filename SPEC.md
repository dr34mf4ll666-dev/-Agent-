...
P1 已把 Graph 事件 seam 下沉到四个 Specialist、C1 综合决策和 C3 外层 Graph，客户页显示 17 个真实节点与条件分支。Specialist/C3 两层 Checkpoint 支持只执行失败或未完成节点；JSON 任务记录支持重启自动续跑和成功报告恢复；任务超时后迟到结果不能覆盖失败状态。P3 将在此基础上补 SQLite 历史、迁移、检索和报告比较，不再承担 P1 的执行恢复缺口。完整说明见 `docs/analysis-jobs.md`。
...
当前调整后项目范围已经完成，没有未关闭的正式任务。客户分析前台、19 项团队验收后台、DeepSeek 解释层和动态辩论固定评测已经形成统一入口；真实模型质量结论只有在用户提供有效 Key 并运行固定评测后才能成立。后续可以继续积累真实模拟账本、扩展更多股票和匹配的行业数据，但这些不影响本次 D4 完成结论。完整边界和最终报告见 `docs/final-delivery.md`，两个 Web 界面分别见 `docs/client-app.md` 和 `docs/control-desk.md`。
