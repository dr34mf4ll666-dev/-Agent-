...
截至 2026-08-17，任务 1.1–1.4、2.1–2.2、3.1–3.3 和 4.1–4.3 的调整后范围均已完成。C3 已通过 Checkpoint 恢复和 20 只真实股票同批次端到端验收；D1 已通过历史时点门禁、完整市场约束和固定多股票真实行情实验；D2–D3 已通过可观测、独立评估、运行级熔断、最小权限和固定对比实验；D4 已完成本地模拟撮合、统一验收与时间豁免记录。产品化补强 P1–P9 已完成当前计划范围，P9 又完成了版本化证券目录、行业筛选和五粮液真实非银行完整 Graph 验收。
...
当前 A1–A5、B1–B2、C1–C3、D1–D4 的调整后交付范围全部完成。`verify-all` 负责命令行稳定复现；客户前台 `/` 负责展示 K 线、四维研究、综合结论和风险边界；团队后台 `/admin` 负责展示全部阶段、19 个操作入口、结果摘要和完整工程证据。DeepSeek 只承担解释层。受约束动态辩论的实现、固定评测集、离线统计和真实 DeepSeek 固定质量门禁均已完成；原始逐次结果覆盖保存到 `.runtime/llm-evaluation/deepseek-fixed-v1.json`。原 1–2 周时间条件被明确豁免且保留未证明声明。第 13 节经项目所有者确认后成为新的产品化主线，但不回改正式任务完成状态。真实交易继续关闭。
...
本节新增条目初始状态为 `planned`。只有项目所有者确认计划，并且对应条目的代码、测试、直观页面、运行入口和文档证据全部通过后，才能改为 `done`；P1–P9 已通过这些条件并标记为 `done`。不按周数推进，严格按照依赖和实际价值排序。
...
| P9 | `done` | 证券主数据与跨行业扩展 | 股票搜索不再局限于 20 只银行股，可按行业筛选并明确哪些数据和能力可用 | 新增 `SecurityMasterRuntime` 与版本化 `security_master.v1.json`；记录代码、交易所、名称、行业、上市状态、分析行业标签、可用来源、能力和快照；兼容非银行财报字段差异 | 新证券不需要修改客户业务源码；行业映射包含银行和酿酒等不同客户行业；五粮液完成真实端到端和行业匹配验收；未验证标的不能进入客户正式目录；客户前台搜索和行业筛选通过 |
...
P1–P9 已完成当前计划范围。P8 在原有部署状态检查上增加 `SecurityRuntime`、登录与角色隔离、CSRF、分级限流、审计和会话 Key；P9 又把客户证券目录迁移到版本化主数据，接入行业筛选，并以五粮液完成第一个非银行真实完整 Graph 验收。当前部署仍默认只服务本机，不开放真实交易；后续扩展更多行业和资产时必须沿用同一主数据与验证边界。
