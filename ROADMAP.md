...
本节新增条目初始状态为 `planned`。只有项目所有者确认计划，并且对应条目的代码、测试、直观页面、运行入口和文档证据全部通过后，才能改为 `done`；P1–P8 已通过这些条件并标记为 `done`，下一项为 P9 证券主数据与跨行业扩展。不按周数推进，严格按照依赖和实际价值排序。
...
P1–P8 已完成。P8 在原有部署状态检查上增加 `SecurityRuntime`、登录与角色隔离、CSRF、分级限流、审计和会话 Key；Docker 使用非 root 用户、只读根文件系统、本机端口和持久卷；CI 自动执行跨平台测试、质量门禁、真实 Chromium 和容器重启恢复。当前部署仍默认只服务本机，不开放真实交易；下一条产品化主线为 P9 证券主数据与跨行业扩展。
