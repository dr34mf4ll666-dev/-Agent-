...
截至 2026-08-20，任务 1.1–1.4、2.1–2.2、3.1–3.3 和 4.1–4.3 的调整后范围均已完成。C3 已通过 Checkpoint 恢复和 20 只真实股票同批次端到端验收；D1 已通过历史时点门禁、完整市场约束和固定多股票真实行情实验；D2–D3 已通过可观测、独立评估、运行级熔断、最小权限和固定对比实验；D4 已完成本地模拟撮合、统一验收与时间豁免记录。产品化补强 P1–P9 已完成当前计划范围，P9 完成了版本化证券目录、行业筛选和五粮液真实非银行完整 Graph 验收；随后独立扩展又完成华银电力真实只读完整 Graph 验收；本轮 P10 完成数据质量、运行指纹、离线可靠性证据和前台可信度展示。
...
当前进度（2026-08-14）：`DynamicDebateRuntime.run(...)` 已接入 Model Gateway、编号化证据目录、Claim/Evidence/Reasoning Schema、确定性证据还原、双方覆盖检查、虚构数字与违规措辞拦截、最多两次语义重试和固定模板降级；客户页面提供手动触发入口并展开两轮结果。真实模型评测暴露出模型会在论证文字中写入日期片段或四舍五入数字，已通过提示词约束修复，继续保持严格数字校验。真实 DeepSeek 固定评测 4 次运行通过：候选/最终证据有效率 100%、正反平衡率 100%、重试率 0%、降级率 0%、结果稳定性 100%，Token 7911；逐次原始结果已覆盖保存到 `.runtime/llm-evaluation/deepseek-fixed-v1.json`。`DYNAMIC_LLM_DEBATE` 与 P7 正式标记为 `done`。
...
当前 A1–A5、B1–B2、C1–C3、D1–D4 的调整后交付范围全部完成。`verify-all` 负责命令行稳定复现；客户前台 `/` 负责展示 K 线、四维研究、综合结论、可信度和风险边界；团队后台 `/admin` 负责展示全部阶段、21 个操作入口、结果摘要和完整工程证据。DeepSeek 只承担解释层。受约束动态辩论的实现、固定评测集、离线统计和真实 DeepSeek 固定质量门禁均已完成；原始逐次结果覆盖保存到 `.runtime/llm-evaluation/deepseek-fixed-v1.json`。原 1–2 周时间条件被明确豁免且保留未证明声明。第 13 节经项目所有者确认后成为新的产品化主线，P10 已完成但不回改正式任务完成状态。真实交易继续关闭。
...
## 13. 产品化补强计划（已确认并完成当前轮）

### 13.1 定位和状态

本节不改变 A1–D4 已完成的正式任务状态，也不把产品增强冒充任务书缺口。当前项目已经具备完整分析能力，接下来的重点从“继续增加 Agent 和指标”转向“让一次分析可以被创建、观察、保存、比较、解释和恢复”。

本节新增条目初始状态为 `planned`。只有项目所有者确认计划，并且对应条目的代码、测试、直观页面、运行入口和文档证据全部通过后，才能改为 `done`；P1–P10 已通过这些条件并标记为 `done`。不按周数推进，严格按照依赖和实际价值排序。
...
| P5 | `done` | 统一可观测性和可靠性面板 | 客户运行中看到节点与整次实际耗时；失败时看到处理办法、失败步骤续跑和 `trace_id`；后台可按一次分析查看瀑布、慢节点、来源状态、缓存/降级和模型成本 | 已新增 `AnalysisObservabilityRuntime` 与内存/JSON adapter；同一 `trace_id` 写入 HTTP、任务、数据快照、17 个 Graph 节点、Harness 输出复核、模型解释和数据库归档；结构化 span 确定性聚合成功率、P50/P95、数据源失败率、缓存命中率、重试/降级率和 Token | 客户错误、任务结果和历史归档共用追踪号；敏感字段拒绝与二次脱敏测试通过；后台瀑布可回答慢点、失败点和降级；离线故障注入固定产生数据失败、两次尝试、缓存降级与 Graph 失败证据 |
| P6 | `done` | 研究工作台与产品闭环 | 客户可持久化自选、收藏与筛选冻结报告、比较同股前后变化或两股横向差异，并按普通版/专业版打印或下载独立 HTML；报告同时标明当前、过期、数据完整、来源降级或部分可用状态 | `ResearchWorkspaceRuntime` 以 `snapshot/toggle_watchlist/toggle_favorite/compare/export_report/export_comparison` 六个 interface 隐藏 JSON 偏好迁移、冻结报告读取、双层投影、状态判定与无模型导出；只读复用 P3/P4，不重新调用数据源、Graph 或模型 | 自选、收藏、筛选、同股/跨股比较、普通/专业导出、来源与风险边界、部分数据/过期状态、HTTP 下载和无模型边界均有自动化证据；完整回归 375 项与产品验收通过 |
| P7 | `done` | LLM 治理与真实质量门禁 | 客户页面已明确显示模型、是否降级、Token、解释版本并允许反馈；团队后台新增模型治理面板，显示两条路由的版本、预算、缓存和固定评测门禁；模型不可用不影响确定性报告 | 已接入 `ModelGovernanceRuntime`：Prompt/模型/Schema 版本、调用与 Token 预算、成功缓存、模型路由和安全降级；新增 `GET /api/governance`；SQLite 保存解释反馈；`LLMQualityGateRuntime` 与 `ModelReleaseRegistry` 提供固定评测门禁和回滚；动态辩论真实路由也经过预算治理；真实评测入口默认覆盖保存唯一原始结果文件 | 离线治理、预算拒绝、缓存、降级、反馈、Mock 不得通过 `require_live` 门禁和候选回滚、后台面板及 HTTP 展示均有证据；真实 DeepSeek 固定评测 4 次运行通过，候选/最终证据有效率 100%、正反平衡率 100%、重试率 0%、降级率 0%、结果稳定性 100%；LLM 仍不能修改指标、价格区间、仓位和风控 |
| P8 | `done` | 正式部署、安全和质量门禁 | 新增登录页和账户窗口；客户只能使用研究前台，管理员才能进入 `/admin` 并查看会话、拒绝事件、限流、模型 Key 状态和安全审计；前后台继续显示版本与服务状态 | `SecurityRuntime` 隐藏密码哈希、会话、角色、CSRF、分级限流、JSONL 审计和会话 DeepSeek Key；`DeploymentRuntime` 负责启动阻断与状态；Docker 非 root/只读/持久卷运行；CI 覆盖 Linux/Windows、Ruff、mypy、覆盖率、密钥与依赖扫描、Chromium E2E、镜像构建和重启恢复 | 登录与 403 角色隔离、CSRF、限流、密钥不持久化/不入审计、非 root UID 10001、真实 Chromium、镜像构建和“任务提交后容器重启仍成功恢复”均已实际通过；完整 P8 中文验收与产品总验收已接入 |
| P9 | `done` | 证券主数据与跨行业扩展 | 股票搜索不再局限于 20 只银行股，可按行业筛选并明确哪些数据和能力可用 | 新增 `SecurityMasterRuntime` 与版本化 `security_master.v1.json`；记录代码、交易所、名称、行业、上市状态、分析行业标签、可用来源、能力和快照；兼容非银行财报字段差异 | 新证券不需要修改客户业务源码；行业映射包含银行、酿酒和电力等不同客户行业；五粮液与华银电力完成真实端到端和行业匹配验收；未验证标的不能进入客户正式目录；客户前台搜索和行业筛选通过 |
| P10 | `done` | 数据可信度、可复现与可靠性证据 | 普通版新增“本次分析可信度”；专业版展示来源/时间、备用或缓存状态、版本输入和运行指纹；比较结果直接说明“为什么不同” | 新增 `DataQualityRuntime.evaluate(snapshot, security_record, now)`、`AnalysisRunIdentity` 和离线 `OfflineReliabilityExperimentRuntime`；SQLite schema v4 新增质量/指纹归档；历史报告使用兼容投影；管理员入口复用固定离线演示；Chromium 入口实际完成两次离线分析验收 | 完整、备用源、缓存、关键缺失、来源/时间不一致、指纹稳定性和旧报告读取均有测试；CLI 一次展示正常、超时重试、缓存降级、Checkpoint 恢复、输出拒绝及 P50/P95/P99；真实 Chromium 检查普通/专业前台、加载结束、手机布局和历史比较的差异说明；不联网、不保存密钥和完整行情 |
...
5. **面试展示证据：P10。** 在数据、历史、观测和主数据稳定后，统一解释可信度、复现版本、故障恢复和报告差异；不靠新增 Agent 堆功能。

P1–P10 已完成当前计划范围。P8 在原有部署状态检查上增加 `SecurityRuntime`、登录与角色隔离、CSRF、分级限流、审计和会话 Key；P9 又把客户证券目录迁移到版本化主数据，接入行业筛选，并以五粮液和华银电力完成两个非银行真实完整 Graph 验收；P10 将质量状态、运行指纹、离线可靠性实验和差异说明接入报告与前台。当前部署仍默认只服务本机，不开放真实交易；后续扩展更多行业和资产时必须沿用同一主数据与验证边界。
