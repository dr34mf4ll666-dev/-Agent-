...
截至 2026-08-13，任务 1.1–1.4、2.1–2.2、3.1–3.3 和 4.1–4.3 的调整后范围均已完成。C3 已通过 Checkpoint 恢复和 20 只真实股票同批次端到端验收；D1 已通过历史时点门禁、完整市场约束和固定多股票真实行情实验；D2–D3 已通过可观测、独立评估、运行级熔断、最小权限和固定对比实验；D4 已完成本地模拟撮合、统一验收与时间豁免记录。当前没有未关闭的正式任务，后续工作进入第 13 节产品化补强计划。
...
本节新增条目初始状态为 `planned`。只有项目所有者确认计划，并且对应条目的代码、测试、直观页面、运行入口和文档证据全部通过后，才能改为 `done`；已经通过这些条件的 P1 现为 `done`。不按周数推进，严格按照依赖和实际价值排序。
...
| P2 | `planned` | 统一分析快照与数据源健康 | 报告显示“本次分析统一数据时点”、每类数据的新鲜度、缓存/真实状态和明确的降级说明；关键数据缺失时允许展示可信的部分结果 | 新增不可变 `AnalysisSnapshot` 和 `AnalysisSnapshotRuntime.acquire(query)`；行情、四 Agent、K 线、DeepSeek 解释和报告消费同一快照；数据源策略统一处理主源、备用源、缓存、熔断和 `not_available` | 同次分析不再为 Graph 和图表重复抓取同类行情；所有下游结果引用同一 `snapshot_id`；来源、`timestamp`、`as_of` 可追溯；主源失败、备用源成功、缓存降级和关键数据全失效均有测试 |
| P3 | `planned` | SQLite 持久化与历史报告 | 新增“最近分析”，可重新打开历史报告、查看当时数据版本和任务状态；服务重启后历史仍存在 | 定义 `AnalysisRepository` interface，提供 SQLite adapter 和内存测试 adapter；保存任务、快照、Agent 结果、Graph 状态、模型调用和报告版本；数据库迁移显式版本化 | 重启进程后可恢复任务和报告；不同报告不会互相覆盖；写入失败不产生半份结果；迁移、并发写入、损坏数据和恢复测试通过；敏感 Key 永不入库 |
| P4 | `planned` | 普通版/专业版、专业图表和证据下钻 | 默认普通版把 17 个真实节点折叠为“准备数据→多维研究→风险检查→生成报告”，结果只突出通俗结论、主要理由、支持因素、风险因素、研究区间、数据时间和免责声明；专业版按需展开四 Agent、完整节点、技术指标、来源与评分规则，并提供十字线、OHLC/成交量提示、缩放、周期和指标开关 | 增加只读 `ReportViewRuntime.project(report_id, view)`，`view` 只允许 `basic/professional`；module 从同一冻结报告和 P2 快照生成两种展示投影与证据索引，不让浏览器拼接业务事实，也不因切换模式重新运行 Graph、数据源或 LLM | 首次访问默认普通版并记住用户选择；普通版不出现 Graph/Harness 等工程术语；专业版能查看 17 个真实节点、四 Agent、指标与证据；两种模式的 `report_id`、`snapshot_id`、结论、分数、区间和风控完全一致，来回切换不新增数据请求、分析任务或 Token；图表与 Agent 数据同源、核心观点可下钻；键盘、非颜色提示、移动端和两模式浏览器验收通过 |
...
P1 已完成。任务执行恢复使用轻量 JSON 与 Graph Checkpoint；P3 仍负责更完整的 SQLite 历史、迁移、检索和报告比较，不重复 P1 的执行职责。下一步进入 P2，保证 Graph、K 线和解释层消费同一批不可变数据快照。
