...
本节新增条目初始状态为 `planned`。只有项目所有者确认计划，并且对应条目的代码、测试、直观页面、运行入口和文档证据全部通过后，才能改为 `done`；已经通过这些条件的 P1、P2 现为 `done`。不按周数推进，严格按照依赖和实际价值排序。
...
P1、P2 已完成。任务执行恢复使用轻量 JSON 与 Graph Checkpoint，并在任务目录保存不可变快照；P3 仍负责更完整的 SQLite 历史、迁移、检索和报告比较，不重复 P1 的执行职责或 P2 的数据采集职责。下一步进入 P3。
