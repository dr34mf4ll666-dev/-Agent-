...
异步任务首次运行时把完整快照原子保存到该任务的 Checkpoint 目录。失败重试或服务重启恢复时读取原快照，不重新抓数据，因此已完成节点和未完成节点始终属于同一 `snapshot_id`。现已完成的 P3 会在任务成功时把这份快照与报告、四 Agent 和 Graph 一起归档到 SQLite；P2 本身仍只负责数据冻结与健康状态。
